...
# Define the linear regression model (replace "target" with your actual target variable)
nullmodel <- lm(price ~ 1, data = train_data[, -1])
model <- lm(price ~ ., data = train_data[, -1])

# Define the stopping criterion to find the top 100 features
stop_criteria <- function(curr_model) {
  selected_features <- names(summary(curr_model)$coefficients[-1, , drop = FALSE])
...
# Perform forward selection to find the top 100 features
forward_result <- stepAIC(nullmodel, direction = "forward", trace = FALSE, scope = list(lower = nullmodel, upper = model), criterion = stop_criteria)
